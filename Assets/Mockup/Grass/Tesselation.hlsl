...
{
    float3 positionWS = (patch[0].positionWS + patch[1].positionWS + patch[2].positionWS) / 3;
    float3 vtcam = _WorldSpaceCameraPos - positionWS;
    float dist = length(vtcam);
    float t = easeOutExpo(saturate(dist / 100));
    float tesselationUniform = _TessellationUniform;//lerp(_TessellationUniform, 1, t);

	TessellationFactors f;
	f.edge[0] = tesselationUniform;
	f.edge[1] = tesselationUniform;
	f.edge[2] = tesselationUniform;
	f.inside = tesselationUniform;
	return f;
}
